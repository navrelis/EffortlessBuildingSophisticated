...
public abstract class AbstractSimiContainerScreen<T extends AbstractContainerMenu> extends AbstractContainerScreen<T> {
	...
	protected void renderForeground(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
		...
		for (GuiEventListener listener : children()) {
			if (listener instanceof Renderable widget && widget instanceof AbstractSimiWidget simiWidget && simiWidget.isMouseOver(mouseX, mouseY)) {
...
//	}
	...
	public boolean keyPressed(int pKeyCode, int pScanCode, int pModifiers) {
		if (getFocused() instanceof EditBox && this.minecraft.options.keyInventory.matches(pKeyCode, pScanCode))
			return false;
