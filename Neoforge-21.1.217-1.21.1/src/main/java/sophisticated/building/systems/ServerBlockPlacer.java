...
//region Delays
    ...
    private long scheduleReplaceMining(Player player, BlockSet blocks, long clientPlaceTime) {
        ...
        if (player instanceof ServerPlayer serverPlayer) {
            PacketDistributor.sendToPlayer(serverPlayer, new BreakCountdownPacket((int) Math.max(0, placeTime - now), replaceCount, true));
        }
        return placeTime;
    }
...
//endregion
    ...
    public void breakBlocks(Player player, BlockSet blocks) {
        ...
        if (player instanceof ServerPlayer serverPlayer) {
            PacketDistributor.sendToPlayer(serverPlayer, new BreakCountdownPacket(delay, blockCount, false));
        }
    }
