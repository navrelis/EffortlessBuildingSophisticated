...
- **runSmokeClient** starts a real client: muted, moved to a secondary monitor if there is one, and deaf to real
  ...
  created; on Forge the smoke client task first writes `earlyWindowControl = false` into the run's `config/fml.toml`,
  so FML's early loading window, which is created before any mod code and takes the focus, is not used and the game
  creates its window itself), is marked inactive for the whole run so the game never grabs, hides or warps the cursor
  (`ClientWindow#keepOffTheCursor`: focus callback removed, `Minecraft#windowActive` false, Windows `WS_EX_NOACTIVATE`),
  and the harness moves only the game's own pointer (`MouseHandler#xpos/ypos`) and calls the input handlers directly.
  On the title screen the harness creates a fresh superflat world with a unique name
...
1.20.1 only `List#getLast` changed (Java 17: `get(size() - 1)`) and the fixture keeps its reflective wrapper lookup;
everything else is unchanged and works on both loaders: `MouseHandler` `xpos`/`ypos`/`onPress`/`onScroll` (Mojang
names in both dev runtimes, MDG Legacy included), `KeyboardHandler#keyPress`, `AbstractWidget#getX/getY`,
`isHovered()`, the mod's screen and widget fields, and the Sophisticated Core GUI classes (`StorageScreenBase`,
`SettingsTabControl`, `ButtonBase`, `ToggleButton`, `WidgetBase`) on Forge 1.20.1 and the Fabric port.
...
- GUI checks (H5): every screen works on both loader builds at the first run (client 21/21 on Forge, 21/21 on Fabric
  ...
  Server runs unchanged (9/9 on both). Cosmetic, original code: the leather, golden and diamond bag titles are wider
  than their GUI texture and run past its right edge; `PlayerSettingsGui` is a render-only stub.
- R2 (ported from mc/1.21.1 fb10ef2..766d18f): player settings editor (radial button + unbound key), config set/save
  on both loaders (Forge 47 `ForgeConfigSpec.ConfigValue#set`/`getDefault` and `ForgeConfigSpec#save()` exist), bag
  title fit and bag name, the six unused widget classes deleted, Terrain Mound option icons (1.21.1's `icons.png`, same
  atlas layout), radial side button layout, the dead mini preview method deleted, and the gameplay fixes (merge-undo
  refund, failed placements not charged, full item count charged, stuck undo stack, Disable + Quick Replace preview).
  Harness: the smoke client never touches the OS cursor or the focus (`ClientWindow#keepOffTheCursor`,
  `SmokeWindowMixin` on both loaders; Forge 47.1 has FML's early window, so its smoke client task writes
  `earlyWindowControl = false` and the mixin targets `ImmediateWindowHandler.setupMinecraftWindow`, registered with
  `--mixin.config` in the MDG Legacy run). Version differences to 1.21.1: `PlayerSettingsGui`'s list uses the 1.20.1
  list API (`ContainerObjectSelectionList(minecraft, width, height, y0, y1, itemHeight)`, `render` instead of
  `renderWidget`, `x0/y0/x1/y1`) and draws the translucent background of the modifier settings list instead of the
  dirt (`setRenderBackground(false)`, `setRenderTopAndBottom(false)`), the screen calls `renderBackground` itself (1.20.1
  screens draw none); `ChargeGameTest` uses `Blocks.GRASS`/`Items.GRASS` (`SHORT_GRASS` from 1.20.3); the harness
  round-trips packets through their `FriendlyByteBuf` constructors and names the bag with `ItemStack#setHoverName`;
  the harness mixin configs use `JAVA_17`. Candles and pink petals exist on 1.20.1, so `MergeUndoGameTest` runs all
  three tests. Verified: `gradlew build` Fabric 104 tests, Forge 90; Fabric `runGametest` "All 24 required tests
  passed"; `runSmokeServer` Fabric 11 (1 skip: `server.refused_place_not_charged`, no place event), Forge 11/11.
  `runSmokeClient` pending (no game clients until the lead allows them).
...
- R3 (5.0.1, ported from mc/1.21.1 d8ab383..48261e8): server checks of build requests (`BuildLimits`,
  ...
  `PowerLevel#serializeNBT()` without registries; item tooltips keep the 1.20.1 `appendHoverText(ItemStack, Level,
  ...)` signature; the Fabric mixin config uses `JAVA_17`; `PlayerDataGameTest` creates players without
  `ClientInformation` (1.20.2+); the smoke players are moved with the 5-arg `moveTo` (Forge's FakePlayer turns the
  1-arg one into a connection teleport that its packet-dropping connection never completes); the Forge smoke test
  uses the template name `smoketest_empty`; `new ResourceLocation` and `get(0)` in the tests. Verified: `gradlew build`
  Fabric 117 tests, Forge 103; Fabric `runGametest` "All 38 required tests passed"; `runSmokeServer` Fabric 12 (1
  skip), Forge 12/12, and with `-PsmokeNoSb=true` Fabric 12 (7 skips), Forge 12 (6 skips);
  `scripts/check-fabric-no-sb-bytecode.ps1 -Mc 1.20.1` on main: 350 classes, 0 differ.
