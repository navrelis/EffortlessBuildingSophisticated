...
- `ClientDriver`: the 1.21.9 input records. A click calls `MouseHandler#onButton(handle, new MouseButtonInfo(button, 0),
  ...
  `Window#handle()` (1.21.10; the same signatures on 26.1.2).
...
- Unchanged and working: the rest of `GuiScenarios` (the modifier list reworked on 1.21.10 still exposes its entries
  ...
  26.1's `GuiGraphicsExtractor`/`extractRenderState` only concern the mod's screens) and `SophisticatedBackpacksScreens`
  (Sophisticated Core 26.1.2 GUI API identical; NeoForge only).
...
So `neoforge-26.1/` builds a second NeoForge jar, `sophisticatedbuilding-neoforge-26.1-5.0.1.jar` (Minecraft
...
`sophisticatedbuilding-neoforge-26.1-4.3.0.jar` (SHA-256 `b05f9eee...`; again with the round-2 jar `74227f37...`), Backpacks 3.25.48 and
Core 1.4.26 in `mods/`:
...
- Round 2 (ported from mc/1.21.1 fb10ef2..766d18f through the mc/1.21.10 and mc/1.21.11 ports): client config
  set/save API and the `PlayerSettingsGui` editor (radial button, unbound "Open Player Settings" key), bag title fit +
  bag name, dead widgets removed (`GuiCheckBoxFixed`, `GuiIconButton`, `GuiNumberField`), Terrain Mound icons +
  highlight, `RadialButtonLayout`, gameplay fixes (merge-undo refund, no charge for failed placements, full-count
  charge, stuck undo stack, Disable + Quick Replace preview), cursor/focus-safe smoke client. 26.1 differences: the new
  screen code uses the render-state GUI (`GuiGraphicsExtractor`: `PlayerSettingsGui#extractRenderState`, its list's
  `extractWidgetRenderState`, rows in `extractContent`, `text`/`centeredText`; the bag screens draw the title in
  `extractLabels` through `BagTitle` and add its tooltip in `extractRenderState`; `LabeledScrollInput` scales long
  numbers with `text`). Harness: `Minecraft#windowActive` is gone (`isWindowActive()` reads `Window#isFocused()`), so
  `ClientWindow#keepOffTheCursor` sets `Window#focused` to false; LWJGL 3.4's `User32.SetWindowLongPtr` takes a
  GetLastError buffer first (null); the window is created in the static `Window#createGlfwWindow` (vanilla and NeoForge;
  NeoForge takes over FML's early window there when there is one), so the Fabric and NeoForge `SmokeWindowMixin`
  inject there (static handler), and Forge 64 calls `ImmediateWindowHandler.setupMinecraftWindow(int, int, String,
  long, Supplier)` (new descriptor). `neoforge-26.1/` needed no new override: round 2 changes none of its overridden
  files, and the NeoForge 26.1 betas create the window the same way. Verified headless (2026-09-25, Gradle on JDK 25):
  `gradlew build` Fabric 104 / NeoForge 90 / Forge 90 / `neoforge-26.1` 90 tests, Fabric `runGametest` "All 25
  required tests passed" (24 + `minecraft:always_pass`), `runSmokeServer` Fabric 5 (`server.refused_place_not_charged`
  skipped: no place event on Fabric), NeoForge 11 (`sb.worn_backpack` skipped as before), Forge 5, `neoforge-26.1` 11
  on NeoForge 26.1.0.19-beta and on 26.1.1.15-beta (`sb.worn_backpack` skipped), all passing. `runSmokeClient` not run
  yet (no game clients until the lead allows them).
