...
Fabric reports 6 checks (1 of them skipped: `server.refused_place_not_charged`), Forge 12 (`forge/` and `forge-1.18/`).
...
- R3 (5.0.1 fixes of `mc/1.21.1` d8ab383..48261e8), ported via mc/1.18.2 (same common code): Fabric per-player data
  ...
  `readVarIntArray`), array limit, Fabric break events and Common Protection API 1.0.0 (built against 1.19, but its
  classes keep the same intermediary names on 1.18.x), offhand bag filter, material cost list, previous build mode,
  translatable texts (`LangKeysTest`). Differences as on 1.18.2: `new TranslatableComponent(...)`, the start reach
  uses the vanilla reach 5 (creative) / 4.5, `PowerLevel#serializeNBT()` without a registry provider, tooltips with the
  `Level` signature, the Omega bag's Reset button is `new Button(...)`, GameTests without `ClientInformation`. Found on
  the way: the new Fabric mixin config came with `compatibilityLevel` `JAVA_21` (now `JAVA_17`); the `FakePlayer` of
  Forge 38 and 39 answers `position()`/`blockPosition()` with the origin, so the server's reach check takes the
  player's coordinates (`getX/getY/getZ`) and the smoke players are moved with the five-argument `moveTo`; the new
  Forge test uses the template `smoketest_empty`, also in the `forge-1.18/` override of `ForgeSmokeServerTests`.
...
- `runSmokeServer`: 12 checks passed (6 `sb.*`). Forge 38 has no game test server, so the task starts a dedicated
