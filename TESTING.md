...
### Porting the H5 checks (GUI screens, 1.21.5 -> 1.21.8)

`GuiScenarios`, its input helpers in `ClientDriver` (`pointAt`/`clickAt`/`scrollAt`/`pressKey`) and `storedLastModifier`'s
1.21.5 NBT reads (`getCompoundOrEmpty`/`getListOrEmpty`/`getStringOr`/`getBooleanOr`/`getIntOr`), `SmokeBackpacks#isBuildingUpgradeEnabled`
and the client-only `SmokeBackpackScreens`/`SophisticatedBackpacksScreens` service ported unchanged from 1.21.5, despite
1.21.6-1.21.8 rewriting GUI *rendering* (`RenderPipelines` instead of `RenderType`, ARGB `drawString` colors, the tooltip
API in `OmegaRandomizerBagScreen#renderWeightBadges`): none of that touches the *fields* the harness reads by reflection
(`MouseHandler#xpos/ypos/onPress/onScroll` are unchanged, `AbstractContainerScreen#leftPos/topPos`, `Screen#renderables`,
the mod's own `addArrayButton`/`closeButton`/`list`/`enableButton`/`countInput` are all still there with the same
declarations as 1.21.5) and the Sophisticated Core GUI API is identical too (checked with `javap` against
`upstream/neoforge/1.21.8/sophisticatedcore-1.21.8-1.5.0.2342.jar`; `UpgradeSettingsTabControl` gained an unrelated
`IForegroundRenderable`/`renderForeground`). NeoForge only.

## Findings (1.21.8)

- Results of the 1.21.8 runs: `runSmokeServer` 3 / 9 (1 skipped: `sb.worn_backpack`, see above) / 3 checks,
  `runSmokeClient` 13 / 21 (8 `sb.*` incl. `sb.upgrade_settings_tab`) / 13 checks on Fabric / NeoForge / Forge, all
  passing (with the H5 GUI checks, H5b 2026-09-25; before them 10 / 17 / 10); `gradlew build` 77 / 65 / 65 tests.
...
- Round 2 (ported from mc/1.21.1 fb10ef2..766d18f, via mc/1.21.4 and mc/1.21.5): client config set/save API and the
  `PlayerSettingsGui` editor (radial button, unbound "Open Player Settings" key), bag title fit + bag name, dead widgets
  removed, Terrain Mound icons + highlight, `RadialButtonLayout`, dead mini preview method removed, gameplay fixes
  (merge-undo refund, no charge for failed placements, full-count charge, stuck undo stack, Disable + Quick Replace
  preview), cursor/focus-safe smoke client. 1.21.8 differences on top of 1.21.5's (test functions + test instances for
  the new server scenarios, Fabric 1.21.5 `@GameTest`, NeoForge `SmokeWindowMixin` on `glfwCreateWindow`): the 1.21.6
  GUI rendering (`GuiGraphics#pose()` is a `Matrix3x2fStack`: `BagTitle` and `LabeledScrollInput` scale with
  `pushMatrix`/`translate(x, y)`/`scale(x, y)`; text colors are ARGB, so the bag titles, the settings screen's title and
  labels and the scaled modifier numbers use `0xFF......`; the settings label tooltip and the bag title tooltip go
  through `GuiGraphics#setTooltipForNextFrame`), and Forge 58's EventBus 7 in the smoke platform
  (`BlockEvent.EntityPlaceEvent.BUS.addListener(event -> refused)`, a cancelling listener). The GameTest player join fix
  (422a074) is kept. Verified headless (2026-09-25): `gradlew build` Fabric 104 / NeoForge 90 / Forge 90 tests, Fabric
  `runGametest` "All 25 required tests passed" (24 + vanilla's `always_pass`), `runSmokeServer` Fabric 5/5
  (`server.refused_place_not_charged` skipped: no place event on Fabric), NeoForge 11/11 (`sb.worn_backpack` skipped as
  before: the fake player has no Curios slot), Forge 5/5. `runSmokeClient` not run yet (no game clients until the lead
  allows them).
