# Sophisticated Building Update – 5.0.1 - Minecraft 1.16.5 and 1.16.4

## Artifacts

* `sophisticatedbuilding-fabric-1.16.5-5.0.1.jar` — Fabric, Minecraft 1.16.4 and 1.16.5
* `sophisticatedbuilding-forge-1.16.5-5.0.1.jar` — Forge, Minecraft 1.16.5
* `sophisticatedbuilding-forge-1.16.4-5.0.1.jar` — Forge, Minecraft 1.16.4
...
* **Fabric, claim and protection mods:** the mod's breaks now fire Fabric API's player block break events (a listener,
  e.g. a claim mod, can cancel them). The Common Protection API does not exist for Minecraft 1.16, so placements are not
  checked against claims on Fabric (Forge's place event already covers them on Forge).
* **Offhand randomizer bag:** a bag in the offhand only supplies the blocks that are in it (it accepted any block
  before).
* **Material cost list:** counts every item a block state needs (e.g. three sea pickles, a double slab) instead of one
  per block, and marks missing items with the same count.
