# Sophisticated Building Update – 5.0.1 - Minecraft 1.16.3
...
* **Fabric: claim and protection mods are respected when breaking.** The mod's breaks fire Fabric API's player break
  events, so a claim mod that protects a block against breaking also keeps it from being broken by the mod. (The
  Common Protection API that newer versions also ask for placements does not exist for Minecraft 1.16.)
