...
public class ProtectionEventsGameTest {
    ...
    private static synchronized void register() {
        ...
        CommonProtection.register(Identifier.fromNamespaceAndPath("sophisticatedbuilding_gametest", "refuse_place"), new ProtectionProvider() {
            @Override
            ...
            @Override
            public boolean canPlaceBlock(Level level, BlockPos pos, NameAndId profile, Player player) {
                return !REFUSE_PLACE.contains(pos);
            }
        });
    }
    ...
    public void cancelledBreakEventKeepsTheBlock(GameTestHelper helper) {
        ...
        try (var config = ConfigScope.baseline()) {
            ...
            expectTrue(helper, BROKEN_AFTER.contains(allowed), "PlayerBlockBreakEvents.AFTER should follow the broken block");
            expectTrue(helper, !(BROKEN_AFTER.contains(refused)), "PlayerBlockBreakEvents.AFTER must not fire for the refused block");
        } finally {
