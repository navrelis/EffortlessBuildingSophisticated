 ...
 */
public class ServerLimitsGameTest implements FabricGameTest {

    private static final BlockState STONE = Blocks.STONE.defaultBlockState();

    @GameTest(template = EMPTY_STRUCTURE)
    public void legitRequestIsScheduled(GameTestHelper helper) {
    ...
    @GameTest(template = EMPTY_STRUCTURE)
    public void startOutOfReachIsRejected(GameTestHelper helper) {
        ServerPlayer player = spawnPlayer(helper, GameType.SURVIVAL);
        try (var config = ConfigScope.baseline()) {
            BlockPos far = player.blockPosition().above(60);
            SophisticatedBuilding.SERVER_BLOCK_PLACER.placeBlocksDelayed(player, set(far, far, List.of(place(far, STONE))), gameTime(helper));
    ...
    @GameTest(template = EMPTY_STRUCTURE)
    public void extentOverTheAxisLimitIsRejected(GameTestHelper helper) {
        ServerPlayer player = spawnPlayer(helper, GameType.SURVIVAL);
        try (var config = ConfigScope.baseline()) {
            BlockPos start = helper.absolutePos(new BlockPos(1, 1, 1));
            ...
            SophisticatedBuilding.SERVER_BLOCK_PLACER.placeBlocksDelayed(player, set(start, end, List.of(place(start, STONE), place(start.east(1), STONE))),
                    gameTime(helper));
    ...
    @GameTest(template = EMPTY_STRUCTURE)
    public void blocksFarFromTheBuildAreRejected(GameTestHelper helper) {
        ServerPlayer player = spawnPlayer(helper, GameType.SURVIVAL);
        try (var config = ConfigScope.baseline()) {
            BlockPos start = helper.absolutePos(new BlockPos(1, 1, 1));
            ...
            SophisticatedBuilding.SERVER_BLOCK_PLACER.placeBlocksDelayed(player, set(start, start, List.of(place(start, STONE), place(far, STONE))),
                    gameTime(helper));
    ...
    @GameTest(template = EMPTY_STRUCTURE)
    public void tooManyBlocksAreCappedToTheLimit(GameTestHelper helper) {
        ServerPlayer player = spawnPlayer(helper, GameType.SURVIVAL);
        try (var config = ConfigScope.baseline()) {
    ...
    private static long gameTime(GameTestHelper helper) {
