...
public final class FabricPlayerData {
    ...
    /** Writes the data into the player's save (1.21.6+: a ValueOutput; the mod's data is one compound under {@link #TAG}). */
    ...
    /** Reads the data back from the player's save. */
    public static void load(Player player, ValueInput input) {
        Holder holder = of(player);
        CompoundTag tag = input.read(TAG, CompoundTag.CODEC).orElseGet(CompoundTag::new);
        if (tag.contains(POWER_LEVEL)) {
            PowerLevel powerLevel = new PowerLevel();
            powerLevel.deserializeNBT(player.registryAccess(), tag.getCompoundOrEmpty(POWER_LEVEL));
            holder.sophisticatedbuilding$setPowerLevel(powerLevel);
        }
