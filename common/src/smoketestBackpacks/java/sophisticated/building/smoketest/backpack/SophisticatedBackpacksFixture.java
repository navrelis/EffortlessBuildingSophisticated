 ...
 * NeoForge builds and the unofficial Fabric port for Minecraft 1.20.4 (their wrapper/upgrade APIs match, except the
 * name of the inventory slot count, see slotCount, and inserting: the Fabric port's inventory only takes items through
 * the Fabric Transfer API, so the contents are put into empty slots with setStackInSlot, which both have); a port whose
...
public final class SophisticatedBackpacksFixture implements SmokeBackpacks {
    ...
    public int count(ItemStack backpack, Item item) {
        InventoryHandler inventory = BackpackWrapper.fromData(backpack).getInventoryHandler();
        int total = 0;
    ...
    /** int getSlots() on NeoForge (IItemHandler), int getSlotCount() on the Fabric port (Porting Lib; its getSlots() is a list). */
