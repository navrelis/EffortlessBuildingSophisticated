 ...
 */
public record TitleFit(float scale, String text, boolean truncated) {
    ...
    public static TitleFit fit(String full, int availableWidth, ToIntFunction<String> width) {
        ...
        int end = full.length();
        while (end > 0 && width.applyAsInt(full.substring(0, end).stripTrailing() + ELLIPSIS) > maxUnscaled) {
            end--;
        }
        return new TitleFit(MIN_SCALE, full.substring(0, end).stripTrailing() + ELLIPSIS, true);
    }
