...
public class RandomizerBagContainer extends AbstractContainerMenu {
	...
	public RandomizerBagContainer(int containerId, Inventory playerInventory, IItemHandler inventory) {
		super(SophisticatedBuilding.RANDOMIZER_BAG_CONTAINER.get(), containerId);
		bagInventory = inventory;

		// Add bag slots - TemplateSlot restricts to 1 item per slot and 1 slot per block type
		for (int i = 0; i < RandomizerBagItem.INV_SIZE; ++i) {
			this.addSlot(new TemplateSlot(bagInventory, i, 44 + (18 * i), 20, RandomizerBagItem.INV_SIZE));
		}
	...
	public boolean stillValid(Player playerIn) {
		return playerIn.getMainHandItem().getItem() instanceof RandomizerBagItem
				|| playerIn.getOffhandItem().getItem() instanceof RandomizerBagItem;
	}
