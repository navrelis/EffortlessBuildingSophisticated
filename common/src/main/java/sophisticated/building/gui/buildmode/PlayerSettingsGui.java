...
public class PlayerSettingsGui extends Screen {
	...
	// No Tooltip class before 1.19.3: the rows report the tooltip under the pointer, render draws it
	...
	protected void init() {
		...
		resetButton = addRenderableWidget(new Button(width / 2 - buttonWidth - 5, buttonY, buttonWidth, 20,
				Component.translatable(LANG + "reset"), button -> resetToDefaults()));
	...
	public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
		// 1.19.2: Screen#render draws no background
		renderBackground(poseStack);
		hoveredTooltip = null;
		super.render(poseStack, mouseX, mouseY, partialTicks);
		drawCenteredString(poseStack, font, title, width / 2, 12, 0xFFFFFF);
		if (hoveredTooltip == null && resetButton.isMouseOver(mouseX, mouseY)) {
			hoveredTooltip = Component.translatable(LANG + "reset.tooltip");
		}
		if (hoveredTooltip != null) {
			renderTooltip(poseStack, font.split(hoveredTooltip, 200), mouseX, mouseY);
		}
	}
	...
	protected class SettingsList extends ContainerObjectSelectionList<Row> {
		...
		SettingsList(Minecraft minecraft, int width, int height, int y, int itemHeight) {
			...
			// Not the dirt background and dirt bands of 1.19.2 lists: a translucent dark background (see render), like the
			// modifier settings list
	...
	protected class HeaderRow extends Row {
		...
		public void render(PoseStack poseStack, int index, int top, int left, int width, int height, int mouseX, int mouseY, boolean hovering, float partialTicks) {
			Font font = rowFont();
			GuiComponent.drawCenteredString(poseStack, font, text, left + width / 2, top + (height - font.lineHeight) / 2 + 1, 0xFFFFAA00);
		}
	...
	public abstract class SettingEntry<T> extends Row {
		...
		SettingEntry(String key, ConfigValue<T> config) {
			...
			this.tooltip = Component.translatable(LANG + key + ".tooltip");
		}
		...
		public void render(PoseStack poseStack, int index, int top, int left, int width, int height, int mouseX, int mouseY, boolean hovering, float partialTicks) {
			...
			font.draw(poseStack, text, left, labelY, 0xFFFFFF);
			boolean overLabel = mouseX >= left && mouseX < left + labelWidth && mouseY >= top && mouseY < top + height;
			boolean overWidget = mouseX >= widget.x && mouseX < widget.x + widget.getWidth() && mouseY >= widget.y && mouseY < widget.y + widget.getHeight();
			if (overLabel || overWidget) {
