 ...
 * <p>
 * Minecraft 1.18 (PoseStack screens): the screen draws the dimmed world itself; the list draws neither its dirt
 * background nor the dirt bands above and below it (clipped to its area instead); there are no widget tooltips, so
 * the screen draws the hovered setting's tooltip after everything else.
 */
...
public class PlayerSettingsGui extends Screen {
	...
	/** The tooltip to draw at the end of this frame (set while the rows and buttons render). */
	@Nullable
	...
	protected void init() {
		...
		Component resetTooltip = new TranslatableComponent(LANG + "reset.tooltip");
		resetButton = addRenderableWidget(new Button(width / 2 - buttonWidth - 5, buttonY, buttonWidth, 20,
				new TranslatableComponent(LANG + "reset"), button -> resetToDefaults(),
				(button, poseStack, mouseX, mouseY) -> pendingTooltip = resetTooltip));
		doneButton = addRenderableWidget(new Button(width / 2 + 5, buttonY, buttonWidth, 20, CommonComponents.GUI_DONE, button -> onClose()));
	}
	...
	public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
		...
		if (pendingTooltip != null) {
			renderTooltip(poseStack, font.split(pendingTooltip, Math.max(width / 2 - 43, 170)), mouseX, mouseY);
		}
	}
	...
	protected class SettingsList extends ContainerObjectSelectionList<Row> {
		...
		SettingsList(Minecraft minecraft, int width, int height, int y, int itemHeight) {
			...
			// 1.18 lists draw a dirt background and dirt bands above and below themselves
		...
		public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
			...
			// Without the dirt bands, rows scrolled past the ends are cut at the list's edges
	...
	public abstract class SettingEntry<T> extends Row {
		...
		public void render(PoseStack poseStack, int index, int top, int left, int width, int height, int mouseX, int mouseY, boolean hovering, float partialTicks) {
			...
			boolean overControl = mouseX >= widget.x && mouseX < widget.x + widget.getWidth() && mouseY >= widget.y
					&& mouseY < widget.y + widget.getHeight();
			if (PlayerSettingsGui.this.list.isInside(mouseY) && (overLabel || overControl)) {
