...
public class PlayerSettingsGui extends Screen {
	...
	public boolean keyPressed(KeyEvent event) {
		...
		if (event.key() != GLFW.GLFW_KEY_UNKNOWN && ClientServices.CLIENT.matchesKey(ClientEvents.keyBindings[ClientEvents.PLAYER_SETTINGS_KEY], event)) {
			onClose();
			return true;
		}
	...
	protected class HeaderRow extends Row {
		...
		public void extractContent(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, boolean hovering, float partialTicks) {
			guiGraphics.centeredText(font, text, getContentXMiddle(), getContentY() + (getContentHeight() - font.lineHeight) / 2 + 1, 0xFFFFAA00);
		}
	...
	public abstract class SettingEntry<T> extends Row {
		...
		public void extractContent(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, boolean hovering, float partialTicks) {
			...
			guiGraphics.text(font, text, left, labelY, 0xFFFFFFFF, false);
			if (mouseX >= left && mouseX < left + labelWidth && mouseY >= top && mouseY < top + height) {
				guiGraphics.setTooltipForNextFrame(font, tooltip.toCharSequence(minecraft), DefaultTooltipPositioner.INSTANCE, mouseX, mouseY, true);
			}
		}
