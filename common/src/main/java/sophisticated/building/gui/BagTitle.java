...
public final class BagTitle {
    ...
    /** Draws the title; call from {@code extractLabels} (coordinates relative to the texture's top left corner). */
    public static void draw(GuiGraphicsExtractor guiGraphics, Font font, Component title, int imageWidth, int color) {
        TitleFit fit = fit(font, title, imageWidth);
        if (fit.scale() >= 1f) {
            guiGraphics.text(font, fit.text(), X, Y, color, false);
            return;
        }
        ...
        guiGraphics.text(font, fit.text(), 0, 0, color, false);
        pose.popMatrix();
    }
    ...
    /** The full title as a tooltip while the mouse is over a cut-off title; call from {@code extractRenderState}. */
    public static void renderTooltip(GuiGraphicsExtractor guiGraphics, Font font, Component title, int left, int top, int imageWidth,
