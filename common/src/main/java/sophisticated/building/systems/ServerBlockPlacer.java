...
//endregion
    ...
    private boolean undoBlockEntry(Player player, BlockEntry block, @Nullable List<BreakToolHelper.ToolSlot> candidates,
        ...
        try {
            switch (action) {
                ...
                case UNMERGE:
                    return unmerge(player, tempBlockEntry, current, candidates != null);
                case REPLACE:
    ...
    private boolean validateRequest(Player player, BlockSet blocks) {
        ...
        double startReach = BuildLimits.startReach(AttachmentHandler.getPlacementReach(player, false), (player.isCreative() ? 5.0 : 4.5) /* Player#blockInteractionRange of 1.20.5+ */);
